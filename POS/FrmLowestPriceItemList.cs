...
namespace POS
{
    public partial class FrmLowestPriceItemList : ComponentFactory.Krypton.Toolkit.KryptonForm
    {
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        dgvItemList.Rows[j].Cells[5].Value = dt.Rows[i]["QtyOnHand"].ToString();
                        ...
                        dgvItemList.Rows[j].Cells[8].Value = dt.Rows[i]["PurchaseCost"].ToString();

                        dgvItemList.Rows[j].Cells[9].Value = dt.Rows[i]["ComparativePrice1"].ToString();
                        dgvItemList.Rows[j].Cells[10].Value = dt.Rows[i]["ComparativePrice2"].ToString();
                        dgvItemList.Rows[j].Cells[11].Value = dt.Rows[i]["ComparativePrice3"].ToString();
                        dgvItemList.Rows[j].Cells[12].Value = dt.Rows[i]["ComparativePrice4"].ToString();
                        dgvItemList.Rows[j].Cells[13].Value = dt.Rows[i]["ComparativePrice5"].ToString();
                        j++;
                    }
                }
                else
                ...
                if (txtItemName.Text != "")
                {
                    ...
                    if (row.Length > 0)
                    {
                        ...
                        for (int i = 0; i < dtNew.Rows.Count; i++)
                        {
                            ...
                            dgvItemList.Rows[j].Cells[8].Value = dtNew.Rows[i]["PurchaseCost"].ToString();
                            dgvItemList.Rows[j].Cells[9].Value = dtNew.Rows[i]["ComparativePrice1"].ToString();
                            dgvItemList.Rows[j].Cells[10].Value = dtNew.Rows[i]["ComparativePrice2"].ToString();
                            dgvItemList.Rows[j].Cells[11].Value = dtNew.Rows[i]["ComparativePrice3"].ToString();
                            dgvItemList.Rows[j].Cells[12].Value = dtNew.Rows[i]["ComparativePrice4"].ToString();
                            dgvItemList.Rows[j].Cells[13].Value = dtNew.Rows[i]["ComparativePrice5"].ToString();
                            j++;
                        }
        ...
        {
            ClsCommon.ObjLowestPriceItemList.Hide();
            ClsCommon.ObjLowestPriceItemList.Parent = null;
            e.Cancel = true;
        }
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                dt = new BALItemMaster().SelectItemByLowestPrice();
                dt.Columns["P4sD"].ColumnName = "P4s-D";

                if (dt.Rows.Count > 0)
                {
                    //princy
                    dt.Columns["Gf"].ColumnName = "NC";
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        if (dr["LowestPrice"].ToString() != "")
                        {
                            ObjItemBOL.LowestPrice = Convert.ToDecimal(dr["LowestPrice"].ToString());
                        }

                        ObjItemBAL.UpdateLowestPrice(ObjItemBOL);
                    }
