...
namespace POS
{
    public partial class FrmPriceTierUpdate : ComponentFactory.Krypton.Toolkit.KryptonForm
    {
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        //dgvItemList.Rows[j].Cells[5].Value = dt.Rows[i]["PriceTier1"].ToString();
                        //dgvItemList.Rows[j].Cells[6].Value = dt.Rows[i]["PriceTier2"].ToString();
                        //dgvItemList.Rows[j].Cells[7].Value = dt.Rows[i]["PriceTier3"].ToString();
                        dgvItemList.Rows[j].Cells[5].Value = dt.Rows[i]["PriceTier1"].ToString();
                        dgvItemList.Rows[j].Cells[6].Value = dt.Rows[i]["PT1Date"].ToString();
                        dgvItemList.Rows[j].Cells[7].Value = dt.Rows[i]["PriceTier2"].ToString();
                        dgvItemList.Rows[j].Cells[8].Value = dt.Rows[i]["PT2Date"].ToString();
                        dgvItemList.Rows[j].Cells[9].Value = dt.Rows[i]["PriceTier3"].ToString();
                        dgvItemList.Rows[j].Cells[10].Value = dt.Rows[i]["PT3Date"].ToString();
                ...
                if (txtItemName.Text != "")
                {
                    ...
                    if (row.Length > 0)
                    {
                        ...
                        for (int i = 0; i < dtNew.Rows.Count; i++)
                        {
                            ...
                            dgvItemList.Rows[j].Cells[5].Value = dtNew.Rows[i]["PriceTier1"].ToString();
                            dgvItemList.Rows[j].Cells[6].Value = dtNew.Rows[i]["PT1Date"].ToString();
                            dgvItemList.Rows[j].Cells[7].Value = dtNew.Rows[i]["PriceTier2"].ToString();
                            dgvItemList.Rows[j].Cells[8].Value = dtNew.Rows[i]["PT2Date"].ToString();
                            dgvItemList.Rows[j].Cells[9].Value = dtNew.Rows[i]["PriceTier3"].ToString();
                            dgvItemList.Rows[j].Cells[10].Value = dtNew.Rows[i]["PT3Date"].ToString();
                            //dgvItemList.Rows[j].Cells[5].Value = dt.Rows[i]["PriceTier1"].ToString();
                            //dgvItemList.Rows[j].Cells[6].Value = dt.Rows[i]["PriceTier2"].ToString();
                            //dgvItemList.Rows[j].Cells[7].Value = dt.Rows[i]["PriceTier3"].ToString();
        ...
        {
            ClsCommon.ObjPriceTierUpdate.Hide();
            ClsCommon.ObjPriceTierUpdate.Parent = null;
            e.Cancel = true;
        }
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                dt = new BALItemMaster().SelectByPriceTier();
                if (dt.Rows.Count > 0)
                {
                    dt.Columns.Remove("ID");
                    //princy
                    dt.Columns["TierGF"].ColumnName = "TierNC";
                ...
                {
                    MessageBox.Show("No found Records");
                }
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        //{
                        //    ObjItemBOL.PriceTier1 = Convert.ToDecimal(dr["PriceTier1"].ToString());
                        //}
                        //if (dr["PriceTier2"].ToString() != "")
                        //{
                        //    ObjItemBOL.PriceTier2 = Convert.ToDecimal(dr["PriceTier2"].ToString());
                        //}
                        //if (dr["PriceTier3"].ToString() != "")
                        //{
                        //    ObjItemBOL.PriceTier3 = Convert.ToDecimal(dr["PriceTier3"].ToString());
                        //}
                        if (dt.Columns.Contains("TierNC") == true)
                        {
                            if (dr["TierNC"].ToString() != "")
                                ObjItemBOL.PriceTier1 = Convert.ToDecimal(dr["TierNC"].ToString());
                        }
                        if (dt.Columns.Contains("TierP4C") == true)
                        {
                            if (dr["TierP4C"].ToString() != "")
                                ObjItemBOL.PriceTier2 = Convert.ToDecimal(dr["TierP4C"].ToString());
                        }
                        if (dt.Columns.Contains("TierMS") == true)
                        {
                            if (dr["TierMS"].ToString() != "")
                                ObjItemBOL.PriceTier3 = Convert.ToDecimal(dr["TierMS"].ToString());
                        }
                        ObjItemBAL.UpdatePriceTier(ObjItemBOL);
                    }
