...
namespace POS
{
    public partial class FrmWebPriceUpdate : ComponentFactory.Krypton.Toolkit.KryptonForm
    {
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        dgvItemList.Rows[j].Cells[5].Value = dt.Rows[i]["WebPrice"].ToString();
                        dgvItemList.Rows[j].Cells[6].Value = dt.Rows[i]["WPDate"].ToString();
                ...
                if (txtItemName.Text != "")
                {
                    ...
                    if (row.Length > 0)
                    {
                        ...
                        for (int i = 0; i < dtNew.Rows.Count; i++)
                        {
                            ...
                            dgvItemList.Rows[j].Cells[5].Value = dtNew.Rows[i]["WebPrice"].ToString();
                            dgvItemList.Rows[j].Cells[6].Value = dtNew.Rows[i]["WPDate"].ToString();
        ...
        {
            ClsCommon.ObjWebPriceUpdate.Hide();
            ClsCommon.ObjWebPriceUpdate.Parent = null;
            e.Cancel = true;
        }
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                dt = new BALItemMaster().SelectByWebPrice();
                if (dt.Rows.Count > 0)
                {
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        if (dr["WebPrice"].ToString() != "")
                        {
                            ObjItemBOL.WebPrice = Convert.ToDecimal(dr["WebPrice"].ToString());
                        }
                        else
                        {
                            ObjItemBOL.WebPrice = 0;
                        }

                        ObjItemBAL.UpdateWebPrice(ObjItemBOL);
                    }
