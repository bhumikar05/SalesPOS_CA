...
namespace POS
{
    public partial class FrmSalesRepSummaryCreditMemoReport : ComponentFactory.Krypton.Toolkit.KryptonForm
    {
        ...
        public FrmSalesRepSummaryCreditMemoReport()
        {
            InitializeComponent();
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            try
            {
                if (ClsCommon.FunctionVisibility("Print", "SalesRepSummaryCreditMemo"))
                {
                    if (CheckValidation())
                    {
                        dtDetail = new DataTable();
                        dtDetail = new BALSalesRepMaster().SalesRepSummaryListCM(new BOLSalesRepMaster() { StartDate = dtpFromDate.Value, EndDate = dtpToDate.Value });
                        ...
                        if (dtDetail.Rows.Count > 0)
                        {
                            ...
                            {
                                DataTable dt = objBal.SelectBySalesRepCM(new BOLSalesRepMaster() { ID = Convert.ToInt32(dr["SalesRepID"].ToString()), StartDate = dtpFromDate.Value, EndDate = dtpToDate.Value });
                                if(dt.Rows.Count > 0)
                                {
                            ...
                            crViewer.Text = "SalesRep CreditMemoSummary Report";
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                if (ClsCommon.FunctionVisibility("Export", "SalesRepSummaryCreditMemo"))
                {
                    if (CheckValidation())
                    {
                        ...
                        if (dtDetail.Rows.Count > 0)
                        {
                            ...
                            if (sfd.ShowDialog() == DialogResult.OK)
                            {
                                ...
                                sXLWorksheet.Cells[1, 1] = "Sales by SalesRep CreditMemoSummary";
